...
const DECISO = [
  ...
  ['features/boss.js', 'attese', 'volatile', 'l\'attesa fra due colpi della stessa persona: dura pochi secondi, e il boss a cui si riferiva col riavvio non c\'e\' piu\''],
  ['features/boss.js', 'silenzi', 'volatile', 'quando ripetere «nessun boss in giro»: al peggio lo si dice una volta in piu\''],
  ['features/colpo.js', 'colpi', 'volatile', 'la banda in raccolta non ha dato monete a nessuno: si pagano solo alla fine, e un riavvio fa saltare il colpo senza che nessuno perda niente'],
  ['features/colpo.js', 'dopo', 'volatile', 'l\'attesa fra due colpi del canale: un riavvio la azzera, e al peggio il colpo dopo si organizza prima'],
  ...
  ['features/instagram.js', '_ultimo', 'volatile', 'com\'e\' andata l\'ultima chiamata vera a Instagram, per canale. Si riscrive a ogni chiamata, e il riquadro, se non trova un esito di pochi minuti fa, ne fa una adesso: dopo un riavvio il registro vuoto non nasconde un collegamento rotto, lo fa solo ricontrollare'],
  ['features/gsi.js', '_colpi', 'volatile', 'lo stesso tetto, per i messaggi che mandano i giochi: riparte da zero e regala un minuto. Tenerlo fuori dal processo costerebbe una scrittura ogni mezzo secondo per ogni gioco acceso, per difendere un contatore da un minuto di troppo'],
  ...
  ['features/posta.js', 'conferme', 'volatile', 'quando ogni canale ha chiesto l\'ultima mail di conferma: dieci minuti di riposo, e dopo un riavvio si puo\' richiedere subito, che non e\' un danno'],
