  ...
  app.get('/api/streamer/piattaforme', requireLogin, wrap(async (req, res) => {
    ...
    const perche = () => {
      ...
      if (kd.ultimoInvio && !kd.ultimoInvio.ok) {
        const i = kd.ultimoInvio;
        const voce = i.come === 'user' ? 'con il tuo account' : (i.come === 'bot' ? "con l'account del bot" : '');
        return `gli eventi arrivano, ma il bot non riesce a scrivere su Kick ${voce}: ${i.motivo}`
          + (i.prossima ? ` — dal prossimo messaggio riprovo ${i.prossima === 'user' ? 'con il tuo account' : "con l'account del bot"}` : '');
      }
      if (kd.ultimoInvio?.ok && kd.ultimoInvio.come) {
        const voce = kd.ultimoInvio.come === 'user' ? 'con il tuo account' : "con l'account del bot";
        return `tutto a posto: il bot scrive ${voce}, ultimo evento ${kd.ultimo ? daQuando(kd.ultimo.quando) : 'poco fa'}`;
      }
...
<p>Finché non confermi, non esce. Non deve uscire?</p>${tastoDecidi(u, chiave, 'ferma', 'Non pubblicare')}${cambia}`;
  ...
  // Prova un'esplosione del muro delle emote nell'overlay, con le emote del canale.
  app.post('/api/muro/prova', requireOwner, wrap(async (req, res) => {
    const login = currentUser(req).login;
    const figura = FIGURE_MURO.includes(req.body?.figura) ? req.body.figura : null;
    if (!figura) return res.status(400).json({ errore: 'figura non valida' });
    try { manager.muro?.prova(login, figura); } catch { /* niente */ }
    res.json({ ok: true });
  }));
  ...
  app.post('/api/streamer/impostazioni', requireLogin, wrap(async (req, res) => {
    ...
    // regola sta in `features/premio.js` ed e' pura — si prova senza accendere
    // un server, e legge da sola le impostazioni del formato vecchio.
    if (b.premioVip !== undefined) out.premioVip = premio.normalizza(b.premioVip, s.settings?.premioVip);
  ...
  app.get('/api/admin/mente3d', requireAdmin, wrap(async (req, res) => {
    ...
    res.json({ moduli, links, vie, vita, plasma: plx.plasma || {}, attivita: plx.attivita || {}, tappe: plx.tappe || [], ambiente: plx.ambiente || null, rete: { nodi: rete.nodi || 0, solidi: rete.solidi || 0, fiducia: rete.fiducia || 0, curiosita: rete.curiosita || 0 } });
  }));
  ...
  //    azioni. SOLO andryxify (il proprietario). Il pubblico non raggiunge MAI questa via.
