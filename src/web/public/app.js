    ...
    '/api/linkpage': { url: 'https://socialbot.live/u/andryxify', pubblicata: true,
      ...
      pagina: { template: 'neon', avatar: '', attiva: true, aggiornata: null,
        ...
        blocchi: [ { tipo: 'link', icona: 'twitch', label: 'Twitch', url: 'https://twitch.tv/andryxify', sotto: 'Ogni sera dalle 21', evidenzia: true },
          ...
          { tipo: 'diretta', piattaforma: 'twitch', canale: 'andryxify', chat: false, autoplay: false, muto: true, titolo: 'Guardami in diretta' },
          { tipo: 'separatore' },
...
async function caricaPaginaLink(ridisegna = false) {
  ...
  box.addEventListener('click', (ev) => {
    ...
    f.type = 'file'; f.accept = 'image/png,image/jpeg,image/webp,image/gif';
    f.onchange = () => conErrore(async () => {
    ...
    if (b.tipo === 'link') {
      ...
      campi = (b.voci || []).map((v, j) => `
        <div class="lp-riga2 spazio-sopra">
          ...
          <input type="url" data-lpb="${i}" data-lpv="${j}" data-lpf="voceUrl" maxlength="${d.limiti.url}" value="${esc(v.url || '')}" placeholder="instagram.com/tuonome">
      ...
      campi = `${b.url ? `<img class="lp-prev" src="${esc(b.url)}" alt="">` : ''}
        <p><button type="button" class="btn secondario mini" data-lpup="${i}">${_bIco(ICO.carica)}${L('Carica un\'immagine', 'Upload an image', 'Subir una imagen')}</button></p>
        <input type="url" class="spazio-sopra" data-lpb="${i}" data-lpf="url" maxlength="${d.limiti.url}" value="${esc(b.url || '')}" placeholder="${esc(L('…oppure incolla un indirizzo', '…or paste an address', '…o pega una dirección'))}">
      ...
      const FORM = { auto: L('Come decide il sito (consigliato)', 'Let the site decide (recommended)', 'Que lo decida el sitio (recomendado)'),
      ...
      campi = `<input type="text" data-lpb="${i}" data-lpf="titolo" maxlength="${d.limiti.label}" value="${esc(b.titolo || '')}" placeholder="${esc(L('Titolo sopra (es. Guardami in diretta)', 'Heading above (e.g. Watch me live)', 'Título arriba (p. ej. Mírame en directo)'))}">
        <label class="campo spazio-sopra">${L('Piattaforma', 'Platform', 'Plataforma')}</label>
        <select data-lpb="${i}" data-lpf="piattaforma">${Object.keys(PIA).map((k) => `<option value="${k}"${(b.piattaforma || 'twitch') === k ? ' selected' : ''}>${PIA[k]}</option>`).join('')}</select>
        <input type="text" class="spazio-sopra" data-lpb="${i}" data-lpf="canale" maxlength="60" value="${esc(b.canale || '')}" placeholder="${esc(b.piattaforma === 'youtube' ? L('ID del canale YouTube (UC…)', 'YouTube channel ID (UC…)', 'ID del canal de YouTube (UC…)') : (mio ? L('il tuo canale: ', 'your channel: ', 'tu canal: ') + mio : L('nome del canale', 'channel name', 'nombre del canal')))}">
        ${(b.piattaforma || 'twitch') === 'twitch' ? `<label class="riga-check spazio-sopra"><input type="checkbox" data-lpb="${i}" data-lpf="chat"${b.chat ? ' checked' : ''}> ${L('Mostra anche la chat sotto al player', 'Show the chat under the player too', 'Mostrar también el chat bajo el reproductor')}</label>` : ''}
...
async function caricaLLM() {
  ...
  box.innerHTML = `
    ...
    ${s.auto ? `<p class="suggerimento">${L('Auto-gestione del modello', 'Model auto-management', 'Auto-gestión del modelo')}: <strong>${s.auto.automatico ? L('attiva', 'on', 'activa') : L('modello forzato da te', 'forced by you', 'forzado por ti')}</strong>${s.auto.libero ? ` · <span class="badge viola">${L('scaletta senza freni', 'no-brakes ladder', 'escalera sin frenos')}</span>` : ''}${s.auto.automatico && s.auto.prossimo ? ` · ${L('prossimo se cala', 'next if it drops', 'siguiente si baja')}: <code>${esc(s.auto.prossimo)}</code>` : ''}${s.auto.chiamate ? ` · ${s.auto.chiamate} ${L('risposte', 'answers', 'respuestas')}, ${L('timeout', 'timeouts', 'timeouts')} ${Math.round((s.auto.tasso_timeout || 0) * 100)}%${s.auto.ms_medio ? `, ~${s.auto.ms_medio}ms` : ''}` : ''}${(s.auto.declassati || []).length ? ` · ${L('scesa a uno più leggero (troppo lenti qui)', 'dropped to a lighter one (too slow here)', 'bajó a uno más ligero (demasiado lentos aquí)')}: ${s.auto.declassati.map((x) => `<code>${esc(x)}</code>`).join(', ')}` : ''}</p><p class="suggerimento">${L('Se il modello non ce la fa (timeout ripetuti) o un download fallisce, il motore scarica DA SOLO uno più leggero/quantizzato, fino a uno che risponde sempre in tempo.', 'If the model cannot cope (repeated timeouts) or a download fails, the engine downloads a lighter/more-quantized one BY ITSELF, down to one that always answers in time.', 'Si el modelo no puede (timeouts repetidos) o falla una descarga, el motor descarga POR SÍ MISMO uno más ligero/cuantizado, hasta uno que siempre responde a tiempo.')}</p>` : ''}
