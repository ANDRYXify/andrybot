...
function corpo(L, l, kick, youtube) {
  ...
  return `
    <section class="vt-scena">
      ...
      <p class="vt-frase">${L('Stanco di doverti iscrivere a un servizio per la chat, uno per gli alert, uno per le donazioni e un altro per i link? Qui c’è tutto, entri una volta sola, e quello che è tuo resta tuo.', 'Tired of signing up to one service for chat, one for alerts, one for donations and another for your links? Here it’s all in one place, you log in once, and what’s yours stays yours.', '¿Cansado de registrarte en un servicio para el chat, otro para los avisos, otro para las donaciones y otro para los enlaces? Aquí está todo, entras una sola vez, y lo que es tuyo sigue siendo tuyo.')}</p>
