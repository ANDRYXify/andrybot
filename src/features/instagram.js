...
export async function puoPubblicare({ userId, token } = {}) {
  const uid = String(userId || '').trim();
  const tok = String(token || '').trim();
  if (!uid || !tok) return { ok: false, motivo: 'Instagram non e\' collegato' };
  const d = await getJson(`${API}/${encodeURIComponent(uid)}/content_publishing_limit?fields=quota_usage&access_token=${encodeURIComponent(tok)}`);
...
export async function pubblicaStoria({ userId, token, url } = {}, { pausa = aspetta } = {}) {
  const uid = String(userId || '').trim();
  const tok = String(token || '').trim();
