...
export async function rispondi({ canale, login, nome, testo, tono, conoscenza, stile, storia, timeoutMs, modo, nomeBot, spunto, lineeGuida, web } = {}) {
  ...
  try {
    const r = await fetch(BASE + '/chat', {
      ...
      body: JSON.stringify({ canale, login, nome, testo, tono, conoscenza, stile, storia, modo, nome_bot: nomeBot, spunto, linee_guida: lineeGuida, web }),
      signal: ac.signal,
    });
    if (!r.ok) return null;
