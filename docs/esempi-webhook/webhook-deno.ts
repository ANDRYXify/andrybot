// ---------------------------------------------------------------------------
// Modulo esterno per SocialBot — Deno, poche righe, ZERO dipendenze.
//
// COME SI AVVIA:  deno run --allow-net webhook-deno.ts   (porta 8099)
//
...
function calcolaRisposta(dati: any): string {
  ...
  return `Ciao ${utente}! Sono il tuo modulo esterno in Deno 🦕`;
}
...
console.log(`Modulo esterno Deno in ascolto sulla porta ${PORT}`);
